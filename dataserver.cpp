...
void process_file_request (FIFORequestChannel* rc, char* request){
	...
	string filename = request + sizeof (filemsg); //grabbing filename from after the filemsg
	filename = "BIMDC/" + filename + '\0'; // adding the path prefix to the requested file name
...
void *handle_process_loop(void *_channel)
{
	...
	for (int i = 0;;){
		...
		delete [] buffer;
        i = 1; //for some reason this starts with an empty message
	}
}
...
int main(int argc, char *argv[])
{
    ...
    cout << "creating channel and entering loop" << endl;
